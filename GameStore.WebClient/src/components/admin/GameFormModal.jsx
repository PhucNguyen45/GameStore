...
export default function GameFormModal({ game, genres, onClose, onSave }) {
  ...
  return (
    ...
    >
      ...
      >
        <h3 style={{ color: "#fff", marginBottom: 20, fontSize: 18, fontWeight: 700 }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: "grid", gap: 12 }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            ...
            <input type="date" placeholder="Release Date" value={form.releaseDate} onChange={(e) => setForm({ ...form, releaseDate: e.target.value })} style={inputStyle} />
            <input placeholder="Developer" value={form.developer} onChange={(e) => setForm({ ...form, developer: e.target.value })} style={inputStyle} />
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <input placeholder="Cover Image URL" value={form.coverImageUrl} onChange={(e) => setForm({ ...form, coverImageUrl: e.target.value })} style={inputStyle} />
            <input placeholder="Trailer URL" value={form.trailerUrl} onChange={(e) => setForm({ ...form, trailerUrl: e.target.value })} style={inputStyle} />
          </div>
          <fieldset style={{ border: "1px solid #1a1a2e", borderRadius: 6, padding: 12 }}>
            <legend style={{ color: "#888", fontSize: 12, padding: "0 8px" }}>System Requirements</legend>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 }}>
              <input placeholder="OS" value={form.minimumOS} onChange={(e) => setForm({ ...form, minimumOS: e.target.value })} style={inputStyle} />
              <input placeholder="Processor" value={form.minimumProcessor} onChange={(e) => setForm({ ...form, minimumProcessor: e.target.value })} style={inputStyle} />
              <input placeholder="Memory" value={form.minimumMemory} onChange={(e) => setForm({ ...form, minimumMemory: e.target.value })} style={inputStyle} />
              <input placeholder="Graphics" value={form.minimumGraphics} onChange={(e) => setForm({ ...form, minimumGraphics: e.target.value })} style={inputStyle} />
              <input placeholder="Storage" value={form.minimumStorage} onChange={(e) => setForm({ ...form, minimumStorage: e.target.value })} style={{ ...inputStyle, gridColumn: "1 / -1" }} />
            </div>
          </fieldset>
          <fieldset style={{ border: "1px solid #1a1a2e", borderRadius: 6, padding: 12 }}>
            <legend style={{ color: "#888", fontSize: 12, padding: "0 8px" }}>Categories (Genres)</legend>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 12 }}>
              {genres && genres.map(g => (
                <label key={g.id} style={{ display: "flex", alignItems: "center", gap: 6, color: "#ccc", fontSize: 13, cursor: "pointer" }}>
                  <input 
                    type="checkbox" 
