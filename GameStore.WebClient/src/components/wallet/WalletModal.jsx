...
import { X, Wallet, CreditCard, ArrowUp, Clock, ArrowDown, ShoppingBag, RotateCcw, Activity } from "lucide-react";
...
const TX_TYPE_META = {
  ...
  Purchase: { icon: ShoppingBag, color: "#e94560", labelKey: "wallet.typePurchase" },
...
function TopUpTab({ user, setUser, onClose, t }) {
  ...
  const handleTopUp = async () => {
    ...
    try {
      ...
      setMessage(`✅ ${t("wallet.success", { amount: formatVND(finalAmount) })}`);
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        <p style={{ color: "#8ab", fontSize: 12, marginBottom: 6, letterSpacing: 1, fontWeight: 600 }}>
          {t("wallet.balance")}
        </p>
        <p style={{ fontSize: 32, fontWeight: 800, color: "#4fc3f7", letterSpacing: 1 }}>
          {formatVND(user?.wallet)}
        </p>
      </div>

      {/* Quick Amounts */}
      <p style={{ color: "#999", fontSize: 12, marginBottom: 10, fontWeight: 600 }}>
      ...
      <div style={{ marginBottom: 16, position: "relative" }}>
        <span style={{ position: "absolute", left: 14, top: 13, color: "#888", fontSize: 15, fontWeight: 600 }}>₫</span>
        <input type="number" placeholder={t("wallet.customAmount")}
          value={customAmount}
          onChange={(e) => { setCustomAmount(e.target.value); setAmount(0); }}
          style={{
            width: "100%", padding: "12px 14px 12px 40px",
            background: "#0d1117", border: customAmount ? "2px solid #4fc3f7" : "1px solid #2a2a4a",
      ...
      {message && (
        <div style={{
          ...
          background: message.includes("✅") ? "rgba(76,175,80,0.12)" : "rgba(233,69,96,0.12)",
          color: message.includes("✅") ? "#4caf50" : "#e94560", fontSize: 13, textAlign: "center",
        }}>{message}</div>
      )}
      ...
      <button onClick={handleTopUp} disabled={loading}
        ...
        {loading ? (          <>
          ...
          <><ArrowUp size={18} /> {t("wallet.topUp", { amount: formatVND(customAmount || amount) })}</>
        )}
      </button>

      {/* Payment Methods */}
      <div style={{ display: "flex", justifyContent: "center", gap: 20, marginTop: 16 }}>
        <span style={{ display: "flex", alignItems: "center", gap: 6, color: "#888", fontSize: 11 }}>
          <CreditCard size={13} /> {t("wallet.card")}
        </span>
        <span style={{ display: "flex", alignItems: "center", gap: 6, color: "#888", fontSize: 11 }}>
          <Wallet size={13} /> GameStore Wallet
        </span>
      </div>
    </div>
  );
...
function HistoryTab({ t }) {
  ...
  const getMeta = (type) => TX_TYPE_META[type] || { icon: Activity, color: "#888", labelKey: "wallet.typeOther" };
        ...
        {typeFilters.map((f) => (
          <button key={f.key} onClick={() => { setTypeFilter(f.key); setPage(1); }}
            style={{
              ...
              border: typeFilter === f.key ? `2px solid ${f.color}` : "1px solid #2a2a4a",
      ...
      {txns.length > 0 && (
        <div style={{
          ...
          {["TopUp", "Purchase", "Refund"].map((type) => {
            ...
            const total = txns.filter(t => t.type === type).reduce((s, t) => s + Math.abs(t.amount), 0);
            return (
              <div key={type} style={{ flex: 1, textAlign: "center" }}>
                <meta.icon size={14} color={meta.color} style={{ marginBottom: 2 }} />
                <p style={{ fontSize: 12, fontWeight: 700, color: meta.color }}>{formatVND(total)}</p>
              </div>
            );
          })}
        </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 4, maxHeight: 320, overflowY: "auto", paddingRight: 4 }}>
        {filtered.length === 0 && !loading && (
          <div style={{ textAlign: "center", padding: "30px 0", color: "#666", fontSize: 13 }}>
            ...
            <p>{typeFilter === "All" ? t("wallet.noTransactions") : t("wallet.noFiltered")}</p>
          </div>
        )}
        ...
        {hasMore && !loading && (
          <button onClick={() => load(page + 1)}
            style={{
              padding: "8px", borderRadius: 8, marginTop: 4,
...
export default function WalletModal({ onClose, initialTab = "topup" }) {
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-content" style={{ width: 460, padding: 0, overflow: "hidden" }}
        ...
        {/* Tabs */}
        <div style={{
          display: "flex", gap: 0, margin: "16px 24px 20px",
          background: "#0d1117", borderRadius: 10, padding: 3,
          border: "1px solid #1e1e3a",
        }}>
